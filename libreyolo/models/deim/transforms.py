"""DEIM training transforms for LibreYOLO fine-tuning.

Built on torchvision.transforms.v2 with the same target contract as the other
LibreYOLO trainers. This is intentionally not a full copy of upstream DEIM's
COCO recipe: Mosaic and collate MixUp are left out until augmentations are
shared across model families. The current transform path includes:
...
- Multi-scale ``DEIMMultiScaleCollate`` collate that randomly resizes each
  batch over a window around 640.
...
def _generate_scales(base_size: int, base_size_repeat: int) -> List[int]:
    """Mirror of ``DEIM/src/data/dataloader.py::generate_scales``.
...
class DEIMTrainTransform:
    ...
    Args:
        ...
        strong_augs: Initial state. The trainer flips this off via
            ``DEIMPassThroughDataset.set_epoch`` once epoch >= stop_epoch.
    """
...
class DEIMPassThroughDataset:
    ...
    def __init__(
        ...
        self.preproc = preproc or DEIMTrainTransform()
    ...
    def set_epoch(self, epoch: int):
        ...
        if self._stop_epoch is not None and epoch >= self._stop_epoch:
            if (
                isinstance(self.preproc, DEIMTrainTransform)
    ...
    def close_mosaic(self):
        ...
        if isinstance(self.preproc, DEIMTrainTransform):
            self.preproc.disable_strong_augs()
